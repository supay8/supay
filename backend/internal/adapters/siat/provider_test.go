...
func TestProviderDecryptAndBuildService(t *testing.T) {
	...
	p12Raw := base64.StdEncoding.EncodeToString([]byte("fake-p12-bytes-content-12345"))
	encToken, _ := cr.EncryptString(token)
	encPass, _ := cr.EncryptString("p12pass")
	encP12, _ := cr.Encrypt([]byte(p12Raw))
...
func TestProviderMissingTokenError(t *testing.T) {
	...
	provider := NewSiatClientProviderWithStorage(&fakeCompanyRepo{company: company}, &fakeCertRepo{cert: cert}, cr, mem, ProviderInfra{BaseURL: "https://example.com", CodigoAmbiente: 2})
...
func TestProviderLocalWithoutStorageFallback(t *testing.T) {
	...
	provider := NewSiatClientProvider(&fakeCompanyRepo{company: company}, &fakeCertRepo{cert: cert}, cr, ProviderInfra{BaseURL: "https://example.com", CodigoAmbiente: 2})
	svc, err := provider.GetForCompany(context.Background(), "comp-1")
	if err != nil {
