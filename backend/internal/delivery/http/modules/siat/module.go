...
func (m *Module) RegisterRoutes(r chi.Router) {
	...
	r.Route("/siat", func(r chi.Router) {
		r.Post("/setup/{pointOfSaleId}", m.h.setup)
		r.Post("/cuis/{pointOfSaleId}", m.h.solicitarCUIS)
		r.Post("/cufd/{pointOfSaleId}", m.h.solicitarCUFD)
		r.Post("/sincronizar/{pointOfSaleId}", m.h.sincronizar)
		r.Post("/evento-significativo/{pointOfSaleId}", m.h.registrarEventoSignificativo)
		r.Post("/firma/{pointOfSaleId}", m.h.firmarFactura)
		r.Post("/paquete/{pointOfSaleId}", m.h.enviarPaquete)
		r.Post("/paquete/{batchId}/validate", m.h.validarPaquete)
		r.Post("/masiva/{pointOfSaleId}", m.h.enviarMasiva)
		r.Post("/masiva/{batchId}/validate", m.h.validarMasiva)
		r.Post("/compras/{pointOfSaleId}", m.h.enviarCompras)
		r.Post("/documento-ajuste/{pointOfSaleId}", m.h.emitirDocumentoAjuste)
		r.Get("/catalogs", m.h.getCatalog)
		r.Get("/catalogs/{tipo}", m.h.getCatalog)

		// Compatibilidad: los handlers comprueban companyId contra el tenant.
