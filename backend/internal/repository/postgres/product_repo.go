...
func (r *PostgresProductRepository) GetByID(companyID, id string) (*domain.Product, error) {
	...
	if err := r.db.Preload("Mappings").First(&m, "id = ? AND tenant_id = ? AND active = true", id, companyID).Error; err != nil {
		return nil, err
	}
...
func (r *PostgresProductRepository) GetBySKU(companyID, sku string) (*domain.Product, error) {
	var m models.Product
	if err := r.db.Preload("Mappings").First(&m, "tenant_id = ? AND sku = ? AND active = true", companyID, strings.TrimSpace(sku)).Error; err != nil {
		return nil, err
	}
	return toDomainProduct(&m), nil
}
...
func (r *PostgresProductRepository) List(companyID string) ([]*domain.Product, error) {
	...
	if err := r.db.Preload("Mappings").Where("tenant_id = ? AND active = true", companyID).Order("sku ASC").Find(&rows).Error; err != nil {
		return nil, err
	}
...
func (r *PostgresProductRepository) UpsertMapping(companyID string, mapping domain.ProductMapping) error {
	return r.db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.First(&product, "id = ? AND tenant_id = ? AND active = true", mapping.ProductID, companyID).Error; err != nil {
			return err
		}
