...
export function InvoiceNewPage() {
  ...
  const posQuery = useQuery({
    ...
    queryFn: () => api.pointOfSale.list(PLACEHOLDER_COMPANY_ID),
    staleTime: 60_000,
  })
  ...
  const emitMutation = useMutation({
    mutationFn: async () => {
      ...
      const draft = await api.invoices.createDraft(payload)
      try {
        const emitted = await api.invoices.emit(draft.id)
  ...
  const retryMutation = useMutation({
    mutationFn: () => api.invoices.emit(resultInvoice!.id),
    onSuccess: (emitted) => {
    ...
    return (
      <div className="mx-auto flex max-w-lg flex-col items-center gap-6 rounded-lg border p-10 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-2">
          <Button
            variant="outline"
            render={<a href={api.invoices.pdfUrl(resultInvoice.id)} download />}
          >
