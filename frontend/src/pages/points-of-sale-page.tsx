...
export function PointsOfSalePage() {
  ...
  const branchesQuery = useQuery({
    queryKey: ["branches"],
    queryFn: () => api.branches.list(PLACEHOLDER_COMPANY_ID),
    retry: false,
  })
  ...
  const toggleMutation = useMutation({
    mutationFn: ({ id, active }: { id: string; active: boolean }) =>
      api.pointOfSale.update(id, { is_active: active }),
  ...
  const reconnectMutation = useMutation({
    mutationFn: (id: string) =>
      api.companies.setup(PLACEHOLDER_COMPANY_ID, { point_of_sale_id: id }),
    onSuccess: () => {
      invalidate()
